// Source: https://github.com/66Ton99/bitburner-scripts/blob/main/hack.js
import {
...
// Dedicated hacking/prep/targeting runner. daemon.js launches this as a separate Netscript process.
// Keep orchestration-only helper launches in daemon.js; this file should only schedule hacking tools.
...
export async function main(ns) {
    ...
    async function startup(ns) {
        ...
        // Helper to determine whether we meet a given home RAM requirement.
    ...
    async function kickstartHackXp(ns) {
        ...
        try {
            ...
            // Ensure we stop studying in case no other running scripts steal focus.
    ...
    async function doRoot(ns, server) {
        ...
        if (ns.getServerMaxRam(daemonHost) <= 8) {
            if (verbose)
                log(ns, `INFO: Deferring root of ${server.name}; 8GB home is reserved for the hack runner.`);
            return false;
        }
        const scriptName = getFilePath('/Tasks/crack-host.js');
        const scriptRam = ns.getScriptRam(scriptName, daemonHost);
        const freeHomeRam = ns.getServerMaxRam(daemonHost) - ns.getServerUsedRam(daemonHost);
        if (freeHomeRam < scriptRam) {
            if (verbose)
                log(ns, `INFO: Deferring root of ${server.name}; ${scriptName} needs ${formatRam(scriptRam)}, ` +
                    `free home RAM is ${formatRam(freeHomeRam)}.`);
            return false;
        }
        const pid = await exec(ns, scriptName, daemonHost, { temporary: true }, server.name);
        if (!pid) return false;
    ...
    async function doTargetingLoop(ns) {
        if (verbose) log(ns, "doTargetingLoop");
        let loops = -1;
        ...
        do {
            ...
            try {
                ...
                if (firstUnpreppedServerIndex !== -1)
                ...
                //       We can add logic to kill lower priority tasks using RAM (such as scripts targetting low priority targets)
    ...
    async function refreshDynamicServerData(ns) {
        ...
        if (ns.getServerMaxRam(daemonHost) <= 8) {
            ...
            dictServerProfitInfo = null;
            log(ns, `WARNING: Could not run analyze-hack.js; using fallback server ordering. ${getErrorInfo(err)}`, false, 'warning');
        }
    ...
    class Server {
        ...
        isPrepped() {
            ...
            this._isPrepped = (currentSecurity == 0 || ((this.getMinSecurity() / currentSecurity) >= 0.99)) &&
                (this.getMaxMoney() != 0 && ((currentMoney / this.getMaxMoney()) >= 0.99));
    ...
    function getPerformanceSnapshot(currentTarget, networkStats) {
        ...
        const jobs = (hackOnly ? [hackCost] : [weaken1Cost, weaken2Cost, growCost, hackCost]).sort((a, b) => b - a); // Sort jobs largest to smallest
    ...
    function optimizePerformanceMetrics(ns, currentTarget) {
        ...
        if (hackOnly)
            newHackThreads = Math.max(1, currentTarget.getHackThreadsNeeded());
        ...
        while (++attempts < maxAdjustments) {
            ...
            newHackThreads = Math.max(newHackThreads + adjustment, hackOnly ? 1 : 0); // Adjust the percentage to steal with pefect precision by actually adjusting the number of hack threads
    ...
    async function performScheduling(ns, currentTarget, snapshot) {
        ...
        for (const schedObj of scheduledTasks) {
            for (const schedItem of schedObj.scheduleItems) {
                ...
                // Args spec: [0: Target, 1: DesiredStartTime, 2: ExpectedEndTime, 3: Duration, 4: LegacyManipulationSlot, 5: DisableWarnings]
    ...
    function getFlagsArgs(toolName, target, allowLooping = true, overrideSilentMisfires = undefined) {
        ...
        if (["hack", "grow"].includes(toolName))
            args.push(0); // Preserve remote arg position used by older grow/hack workers.
