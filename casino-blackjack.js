// Source: https://github.com/66Ton99/bitburner-scripts/blob/main/casino-blackjack.js
import {
...
export async function main(ns) {
    ...
    async function start() {
        ...
        const btnSaveGame = await findCasinoSaveButton(findRequiredElement);
        async function saveGame() { return await saveCasinoGame(ns, click, btnSaveGame, saveSleepTime); }
        ...
        try {
            ...
            while (true) {
                if (priorAttempts > 0)
                    await ns.sleep(1000);
                try {
                    await checkForStolenFocus(false, 3, true);
                    await ensureInAevum(ns, click, findRequiredElement, async () => {
                        const travelled = await getNsDataThroughFile(ns, 'ns.singularity.travelToCity(ns.args[0])', null, ["Aevum"]);
                        if (!travelled)
                            log(ns, "INFO: Canot use singularity travel to Aevum via singularity. We will try to go there manually for now.", true);
                        return travelled;
                    });
                    await navigateToCasino(ns, click, findRequiredElement,
                        async () => await getNsDataThroughFile(ns, 'ns.singularity.goToLocation(ns.args[0])', null, ["Iker Molina Casino"]));
                    await openCasinoGame(click, findRequiredElement, 'blackjack');
                    ...
                    if (ns.ls("home", "Temp/").length > 0) { // If there are some temp files, it suggests we haven't killed all scripts and cleaned up yet
                        ...
                        if (await tryfindElement("//p[contains(text(), 'Count:')]", 10)) { // If this works, we're still allowed in
                            ...
                            if (!(await checkStillAtCasino(false)))
                                throw new Error("Lost casino focus while validating blackjack start after entering the casino.");
                    ...
                    if (++priorAttempts >= 5)
                        throw new Error(`After ${priorAttempts} attempts, casino.js continues to catch unexpected errors ` +
            ...
            return log(ns, `ERROR: casino.js Caught a fatal error after entering blackjack:\n${getErrorInfo(err)}\n${supportMsg}`, true, 'error');
        }
                ...
                switch (winLoseTie) {
                    ...
                    case "win": // We want to "lock in" our wins by saving the game after each one
                        ...
                        if (netWinnings > peakWinnings) {
                            ...
                            if (netWinnings > 0)
                                await saveCasinoGame(ns, click, btnSaveGame, saveSleepTime); // Save if we won
                        }
                        continue;
                    case "lose":
                        netWinnings -= bet;
