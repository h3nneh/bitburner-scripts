...
// Source: https://github.com/66Ton99/bitburner-scripts/blob/main/infiltrate.js
	...
	},
	{
		...
		init: function (screen) {
			...
			for (let i = 0; i < instructions.length; i++) {
				...
				if (-1 !== line.indexOf("cut wire number")) {
					const match = line.match(/cut wire number\s+(\d+)/);
					if (match) wires.push(parseInt(match[1]));
				}
